...
public class ReadFiles {
    ...
    public static void executeStepMethod(String methodName, Object targetInstance) {
        try {
            Method method = targetInstance.getClass().getDeclaredMethod(methodName);
            method.setAccessible(true); // <--- required if method is not public
            method.invoke(targetInstance);
        } catch (Exception e) {
            e.printStackTrace(); // Handle errors or rethrow
        }
    }
